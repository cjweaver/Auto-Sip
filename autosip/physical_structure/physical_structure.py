...
def shelfmark_from_file_json(file_json):
    ...
    Returns:
        str: A single shelfmark that the file name represents e.g. 'C604-1'
    """
    filename_parts = [fn_part for fn_part in file_json['Name'].split('_') if fn_part not in originators]
    return list(filter(lambda filename: re.search("^(?![isfvISFV]{1}[0-9]+)[a-zA-Z0-9-]{1,}", filename), filename_parts))[0]
...
def physical_items_from(files_json, sip_id, item_format, sip_text, shelfmark_order):
    ...
    physical_items = {key:list(group) for key, group in groupby(files_json, lambda file_json: shelfmark_from_file_json(file_json))}
    ...
    for shelfmark in shelfmark_order:
        ...
        #Make sure the item(s) under the shelfmarks are sorted by filename, just in case.
        physical_items[shelfmark].sort(key=lambda  item: item['Name'])
        # Check if there are multiple items under a single shelfmark
        shelfmark_has_items = False
        # multiple_items = [re.findall(r'i\d', item['Name'])[0] for item in physical_items[shelfmark]]
        multiple_items = [re.search(r'i\d', item['Name']).group(0) for item in physical_items[shelfmark] if re.search(r'i\d', item['Name']) != None]
        if len(multiple_items) > 1:
            print("Multiple items under a single shelfmark")
            # Add an item key (i1, i2, etc.) to physical_items
            items = {key:list(group) for key, group in groupby(physical_items[shelfmark], key= lambda item: re.findall(r'i\d', item['Name'])[0])}
        else:
            items = physical_items[shelfmark]
...
sip_physical_structure = physical_items_from(files_json, 66922, "Tape", sip_text, shelfmark_order)

# with open("sip_test.json", "w") as f:
#     f.write(json.dumps(sip_physical_structure))


# patch_physical_structure(64350, sip_physical_structure, physical_step_state_id, user_id)
