...
def saveContinue():
    driver.wait.until(
        EC.element_to_be_clickable((By.CLASS_NAME, "nav-save-button"))
    ).click()
    driver.wait.until(
    ...
    driver.wait.until(
        EC.element_to_be_clickable(
            (By.XPATH, "//*[@id='page-content-wrapper']/div[2]/div[2]/nav/button[3]")
        )
    ).click()
...
def createNewsip(shelfmark, grouping="None"):
    ...
    SAMI_results = driver.wait.until(
        EC.element_to_be_clickable((By.CLASS_NAME, "list-group"))
    )
    ...
    driver.wait.until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/div[3]/div[2]/nav/button[3]"))
    ).click()
    # Return the sip_id
...
def date_tally(filenames):
    ...
    for file in filenames:
        filedate = datetime.strptime(
            (file.partition("Modified: ")[2]), "%a, %d %b %Y %H:%M:%S %Z"
        )
...
def source_files(directory, file_pattern, sip_id):
    ...
    s1 = ui.Select(
        driver.wait.until(EC.element_to_be_clickable((By.ID, "currentSourceBox")))
    )
    ...
    assert (
        not "0 files" in any_files.text
    ), f"Sorry no files found with filename {file_pattern} at {path}"
...
def analysis(sip_id):
    ...
    driver.wait.until(
        EC.element_to_be_clickable(
            (By.XPATH, "//*[@id='copy-files-buttons']/div/div/div/button[1]")
        )
    ).click()
    ...
    def retry_analysis(failure, page, sip_id):
        ...
        if failure == "Analysis":
            ...
            elem = driver.find_element_by_xpath(
                "//a[@href='#analysis-failed-collapse']"
            )
            ...
            failed_files = elem.find_elements_by_xpath(
    ...
    patch_url = "{0}/api/SIPs/SIP/State/{1}".format(site, user_id)
...
def physical_structure(physical_structure_url, sip_id):
    ...
    s1 = ui.Select(
        driver.wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="demo1"]/div/div[1]/div[1]/select')
            )
        )
    )
    ...
    for i in range((len(file_names)) - 1):
        ...
        driver.wait.until(
            EC.element_to_be_clickable(
                (By.XPATH, "//*[@id='demo2']/div/div[1]/div[3]/button[2]")
            )
        ).click()
...
def copy_processmetadata(
    src_sip_id, dest_sip_id, speed, eq, notes, process_metadata_date
):
    ...
    patch_url = "{0}/api/SIP/processmetadata/{1}/{2}/{3}".format(
        site, dest_sip_id, dest_step_id, dest_user_id
    )
    print("patching", patch_url)
    r = requests.patch(patch_url, json=dest_process_metadata, verify=False)
    # print(r)
    ...
    driver.wait.until(
        EC.element_to_be_clickable(
            (By.XPATH, "//*[@id='process-metadata-form']/div[3]/button")
        )
    )
    ...
    print("\n")
    print(
        f"Process Metadata complete for {site}/Steps/Process/{dest_sip_id}/{dest_step_id}"
    )
...
def getSIPStobuild():
    ...
    # Skip first row
    for row in rows:
        shelfmark, grouping, filename, directory, item_format, pm_date, reference_sip, speed, eq, notes = (
            row
        )
...
def main():
    ...
    for sip in SIPS:
        retry_count = 0
        shelfmark, grouping, directory, filemask, item_format, pm_date, reference_sip, speed, eq, notes = (
            sip
        )
        ...
        try:
            ...
            physical_structure(
                physical_structure_url, sip_id
            )  # pass the physical structure??
            copy_processmetadata(
                reference_sip, sip_id, speed, eq, notes, process_metadata_date
            )
